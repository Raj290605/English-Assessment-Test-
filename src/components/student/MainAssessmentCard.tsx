...
export function MainAssessmentCard({ status, answeredCount, totalQuestions }: MainAssessmentCardProps) {
  ...
  return (
    <div className="bg-white rounded-md p-6 border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
      ...
      <div className="flex-1 space-y-2">
        ...
        <p className="text-[13px] text-slate-500 max-w-md">{content.desc}</p>
      ...
      <div className="shrink-0 w-full md:w-auto">
        {status === 'NOT_STARTED' || status === 'IN_PROGRESS' ? (
          <Link
            href="/assessment"
            className="block w-full md:w-auto py-2.5 px-6 bg-white border border-blue-600 text-blue-600 hover:bg-blue-50 font-semibold text-[13px] rounded-md text-center transition-colors shadow-sm"
          >
            {content.actionText}
          </Link>
        ) : (
          status === 'EVALUATED' && (
            <div className="w-full md:w-auto">
              <div className="[&>button]:w-full [&>button]:py-2.5 [&>button]:px-6 [&>button]:bg-white [&>button]:border [&>button]:border-blue-600 [&>button]:text-blue-600 [&>button]:font-semibold [&>button]:text-[13px] [&>button]:rounded-md [&>button]:shadow-sm [&>button]:hover:bg-blue-50">
                <ReattemptButton />
              </div>
            </div>
          )
