...
export function StudentSidebar({ user, isOpen, onClose }: StudentSidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`
        fixed top-0 left-0 bottom-0 z-50 w-[280px] bg-[#0A1930] text-slate-300
      ...
      `}>
        ...
        <div className="p-7">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-xl border border-slate-700/50 bg-white/5 flex items-center justify-center shrink-0">
              <ShieldCheck className="w-5 h-5 text-white" />
            </div>
            <div className="flex flex-col leading-none gap-1">
              <span className="font-bold text-white tracking-widest text-[13px]">CREDIBILITY</span>
              <span className="font-semibold text-blue-400 text-[11px] tracking-widest">INTERVIEW TEST</span>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 px-4 space-y-6 overflow-y-auto">
          ...
          <div className="space-y-1">
            <h4 className="px-4 text-[10px] font-bold text-slate-500 tracking-[0.2em] mb-3 mt-4">ACCOUNT</h4>
