...
// Enforce whole number display for scores /10
...
export function EvaluationSummary({ evaluation, responses, questions = [] }: EvaluationSummaryProps) {
  ...
  return (
    <div className="w-full flex flex-col gap-8">
      ...
      {/* OVERALL SCORE PANEL */}
      <div className="bg-white rounded-3xl p-8 md:p-10 border border-slate-200 shadow-sm relative overflow-hidden flex flex-col items-center justify-center text-center">
        <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-b from-blue-50/40 to-transparent pointer-events-none" />

        <div className="relative z-10">
          <div className="w-16 h-16 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center mb-6 mx-auto border border-blue-100">
            <Award className="w-8 h-8" />
          </div>

          <h3 className="text-sm font-bold text-slate-400 uppercase tracking-[0.2em] mb-2">Overall Score</h3>

          <div className="flex items-baseline justify-center gap-2 mb-2">
            <span className="text-7xl font-extrabold text-slate-900 tracking-tight">
              {formatScore(evaluation.overallScore)}
            </span>
            <span className="text-3xl font-bold text-slate-300">/ 10</span>
          </div>

          <p className="text-sm font-medium text-slate-400 max-w-md mx-auto mt-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          <div key={cat.label} className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex flex-col hover:border-blue-200 transition-colors">
            ...
            <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">{cat.label}</div>
            <div className="flex items-baseline gap-1 mt-auto">
              <span className="text-2xl font-bold text-slate-900 leading-none">{formatScore(cat.score)}</span>
              <span className="text-[13px] font-bold text-slate-300">/ 10</span>
            </div>
          </div>
        ))}
      </div>
                    ...
                    {r.feedback?.score != null && (
                      <div className="flex flex-col items-end shrink-0">
                        ...
                        <div className="flex items-baseline gap-1 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200">
                          <span className="text-lg font-bold text-slate-900">{formatScore(r.feedback.score)}</span>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                          {r.feedback.strengths && (
                            <div className="bg-emerald-50/50 p-4 rounded-lg border border-emerald-100">
                              <h5 className="text-[10px] font-bold text-emerald-700 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                          ...
                          {r.feedback.needsImprovement && (
                            <div className="bg-amber-50/50 p-4 rounded-lg border border-amber-100">
                              <h5 className="text-[10px] font-bold text-amber-700 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                                <TrendingUp className="w-3 h-3" /> Needs Improvement
                              </h5>
